...
1. If OSGi configurations to use microservice are enabled , will the publishing process work on local AEM server with the same codebase?
    * No, if the flag `dxml.use.publish.microservice` is set to `true` then it always look for microservice configurations. Set `dxml.use.publish.microservice` to `false` for the publishing to work on your local.
